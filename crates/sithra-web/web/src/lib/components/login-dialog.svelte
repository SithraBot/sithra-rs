<script lang="ts">
    import { authHeader, encodeCredentials } from "$lib/auth";
    ...
    async function onsubmit() {
        ...
        try {
            let req = await fetch("/auth/authorize", {
                method: "POST",
...
<Dialog.Root bind:open={getOpen, setOpen}>
    ...
    >
        <Dialog.Header>
            <Dialog.Title>登陆</Dialog.Title>
            <Dialog.Description>请使用用户名与密码登陆。</Dialog.Description>
        </Dialog.Header>
        ...
        <Dialog.Footer>
            <Button
                ...
                class="w-full"
                onclick={onsubmit}
                disabled={username.trim() == "" || password == ""}>登陆</Button
            >
        </Dialog.Footer>
    </Dialog.Content>
</Dialog.Root>
